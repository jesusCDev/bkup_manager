...
class BackupManager:
    ...
    def construct_rsync_command(self, main_folder_path: str, backup_folder_path: str, exclude_folder_list: list):
        RSYNC_CMD = f"{Fore.LIGHTBLACK_EX}rsync -avhl --delete --progress --ignore-existing{Style.RESET_ALL} "
        EX_FLAG   = f"{Fore.YELLOW}--exclude{Style.RESET_ALL}"
        
        self.metrics_tracker.main_path = main_folder_path
    
        for ex_folder in exclude_folder_list:
            RSYNC_CMD += f'{EX_FLAG} "{ex_folder}" '
            
            self.metrics_tracker.record_excluded_folder(ex_folder)
        
        return f"{RSYNC_CMD} '{Fore.LIGHTCYAN_EX}{main_folder_path}{Style.RESET_ALL}' '{Fore.LIGHTCYAN_EX}{backup_folder_path}{Style.RESET_ALL}'"
    
    def execute_rsync_command(self, rsync_cmd: str):
        if self.production_run:
    ...
    @staticmethod
    def create_directory_if_not_exists(folder_path: Path):
        folder_path.mkdir(parents=True, exist_ok=True)
    
    def backup_directory(self, main_folder_path: str, backup_folder_path: str, exclude_folder_list: list):
    ...
    def delete_untracked_directories(self, backup_folder_path: Path, directories: list):
    ...
    def backup_drive(self, drive: Drive, backup_path: Path):
        ...
        self.send_notification(f"{drive.drive_name} Completed")
